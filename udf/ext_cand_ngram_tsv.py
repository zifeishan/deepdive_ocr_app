...
arr_cand_word_id = None
arr_candidate_id = None
arr_varid = None
arr_candid = None
arr_feature = None
gram_len = None
...
def DFS(docid, edges, nowid, res_array, index_cwid_sub):
  global arr_candidate_id, arr_feature, gram_len
...
for row in sys.stdin:
  ...
  docid = parts[0]
  # arr_id = obj['arr_id']
  arr_cand_word_id = [x for x in parts[1].split(',')]
  arr_candidate_id = [x for x in parts[2].split(',')]
  ...
  for startid in sorted(edges.keys()):
    ...
    for item in DFS(docid, edges, startid, res_array, index_cwid_sub):
      # yield item
      # docid, cand_word_id, candidate_id, ngram = item
      print '\t'.join([str(_) for _ in item[:3]]) + '\t' + item[3]
