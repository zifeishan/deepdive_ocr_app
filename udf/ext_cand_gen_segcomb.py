...
re_dash2 = re.compile(u'\xe2\x80\x93|\xe2\x80\x94|\xe2\x88\x92')
re_quote2 = re.compile(u'\xe2\x80\x9d|\xe2\x80\x9c')
re_singlequote2 = re.compile(u'\xe2\x80\x99|\xe2\x80\x98')

def Escape(word):
  word = re.sub(re_dash, '-', word)
  word = re.sub(re_quote, '"', word)
  word = re.sub(re_singlequote, "'", word)
  word = re.sub(re_dash2, '-', word)
  word = re.sub(re_quote2, '"', word)
  word = re.sub(re_singlequote2, "'", word)
...
def SplitWordByPunc(s):
  ...
  return [p for p in re.split(split_pattern, s) if p != '']
