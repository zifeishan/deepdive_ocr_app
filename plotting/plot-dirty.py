...
# 100 docs
tesspath = "evaluation/ddocr_dirty/eval-results-tess.txt"
cunipath = "evaluation/ddocr_dirty/eval-results-cuni.txt"
optimalpath = "evaluation/ddocr_dirty/bestpick-optimal-fuzzy/opt.0.txt"
fuzzypath = "evaluation/ddocr_dirty/bestpick-optimal-fuzzy/"
...
opt2path = 'evaluation/ddocr_dirty/bestpick-evalgen.txt'

# fuzzypath = "evaluation/bestpick-optimal-fuzzy-trgm/"
ddpath = 'eval-results-ddocr_dirty.txt'
ddepath = 'eval-results-ddocr_dirty.txt' # TODO!!
# DISTRANGE = range(1, 6)
# DISTRANGE = range(1, 4)
DISTRANGE = range(1, 3)
...
# French docs
exclude_docs = ['JOURNAL_54541', 'JOURNAL_27209', 'JOURNAL_4998' , 'JOURNAL_41200', 'JOURNAL_54604', 'JOURNAL_72508', 'JOURNAL_67458', 'JOURNAL_35876',
  'JOURNAL_53763','JOURNAL_529','JOURNAL_27111','JOURNAL_14068']
...
tess = [ (t[0], float(t[-1].strip('()'))) for t in tess if t[0] not in exclude_docs]
cuni = [ (t[0], float(t[-1].strip('()'))) for t in cuni if t[0] not in exclude_docs]
dd =   [ (t[0], float(t[-1])) for t in dd if t[0] not in exclude_docs]
dde =   [ (t[0], float(t[-1])) for t in dde if t[0] not in exclude_docs]
opt = [ (t[0], float(t[-1].strip('()'))) for t in opt if t[0] not in exclude_docs]
opt2 = [ (t[0], float(t[-1].strip('()'))) for t in opt2 if t[0] not in exclude_docs]
