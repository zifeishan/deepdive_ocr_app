...
def FeatureExtract(candfile):
  cands = [line.strip().split('\t') for line in open(candfile).readlines()]

  options = []
  
  feature_names = []
  if not candfile.endswith('.cand'):
    print 'ERROR: file must ends with .cand!'
    sys.exit(1)
  if outputbase == '':
    filebase = alignedfile[:-len('.cand')] # remove ".outpu..."
    filebase += '.'
  print 'savebase:', filebase
  fout = open(filebase + 'feature', 'w')
  ...
  for c in cands:
    ...
    this_feature_names = []
    # TODO move to udf ...
    fnames, ftrs = featureextractor.CandidateFeatureExtract(options, isBoolean=True)
    this_features = ftrs
